...
def test_nvda_shift_v_announces_both_installed_versions():
	announcements = []
	addon_handler = SimpleNamespace(
		getCodeAddon=lambda: SimpleNamespace(manifest={"version": "5.6.6"}),
	)
	script = _load_version_script(addon_handler, announcements)

	script(SimpleNamespace(app_version="12.9.1.0"), None)

	assert announcements == ["Unigram version: 12.9.1.0. UnigramPlus version: 5.6.6."]
...
def test_version_script_has_the_requested_gesture_and_input_help_description():
	...
	assert keywords["gesture"].value == "kb:NVDA+shift+V"
