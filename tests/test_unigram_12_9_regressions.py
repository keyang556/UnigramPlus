...
def test_saved_messages_tab_list_uses_profile_scrolling_host_not_localized_names():
	namespace = _load_module_members(
		{"_find_ancestor_by_automation_id", "_is_saved_messages_tab_list"},
		{},
	)
	profile_scroll = Node(automation_id="ScrollingHost")
	profile_page = Node(parent=profile_scroll)
	saved_messages = Node(automation_id="Messages", parent=profile_page)
	ordinary_chat = Node(automation_id="Messages", parent=Node())

	assert namespace["_is_saved_messages_tab_list"](saved_messages)
	assert not namespace["_is_saved_messages_tab_list"](ordinary_chat)
	assert not namespace["_is_saved_messages_tab_list"](
		Node(automation_id="ChatsList", parent=profile_scroll)
	)


def test_saved_messages_tab_rows_bypass_the_message_overlay_and_focus_parser():
	...
	file_transfers = SimpleNamespace(active=True, _is_transfer_button=lambda obj: False)
	namespace = _load_module_members(
		{"_find_ancestor_by_automation_id", "_is_saved_messages_tab_list"},
		{
			"Role": role,
			"conf": SimpleNamespace(get=lambda key: False),
			"File_transfer_progress_tracking": file_transfers,
			"is_recording_button": lambda obj: False,
			"log": SimpleNamespace(debug=lambda *args: None),
			"_WINDOW_SURFACE_AUTOMATION_IDS": frozenset(("Messages",)),
			"_SAVED_MESSAGES_TAB_LIST_MARKER": "_savedList",
			"_SAVED_MESSAGES_TAB_ROW_MARKER": "_savedRow",
		},
	)
	choose_overlay = _load_app_method("chooseNVDAObjectOverlayClasses", namespace)
	event_gain_focus = _load_app_method("event_gainFocus", namespace)
	profile_scroll = Node(automation_id="ScrollingHost")
	messages = Node(automation_id="Messages", parent=Node(parent=profile_scroll))
	row = Node(role=role.LISTITEM, parent=messages, name="native accessible summary")
	instance = SimpleNamespace(
		isUnigramWindow=True,
		_remember_messages_button=lambda obj: None,
		_classify_window_surface=lambda obj: "other",
		isSkipName=0,
		isOpenProfile=False,
	...
	choose_overlay(instance, messages, [object])
	classes = [object]
	choose_overlay(instance, row, classes)
	next_handler_calls = []
	event_gain_focus(instance, row, lambda: next_handler_calls.append(True))

	assert getattr(messages, "_savedList") is True
	assert getattr(row, "_savedRow") is True
	assert classes == [object]
	assert row.name == "native accessible summary"
	assert next_handler_calls == [True]
