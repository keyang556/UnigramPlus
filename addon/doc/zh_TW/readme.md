...
* 恢復可靠的語音訊息錄製偵測，並避免可能造成訊息導覽延遲的不必要聊天結尾 UI Automation 探測；個人資料中「已儲存訊息」分頁的清單項目現在保留 Unigram 原生無障礙資訊，不再因完整解析訊息樹而產生延遲。
* 移除了過時的「已儲存訊息」主題名稱暫時解決方法；目前版本的 Unigram 現已原生為「已儲存訊息」聊天提供無障礙名稱。
