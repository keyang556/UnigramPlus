...
* ALT+I：開啟所有聊天搜尋結果的清單。
* shift+F3：在聊天中跳到上一個搜尋結果。
* F3：在聊天中跳到下一個搜尋結果。
* ALT+W：朗讀訊息傳送或接收的時間，以及反應清單。連按兩下可切換此資訊的朗讀模式。

##更新紀錄：

### 5.5.3版

* 新增自動播報檔案上傳與下載進度。在進度列播報設定中新增「僅在上傳和下載期間」選項，並將其設為新的預設值。ALT+U 快捷鍵現在會在三種狀態之間切換：關閉、僅在上傳/下載期間，以及播報所有進度列。
* 更新讀我檔案與翻譯。
...
* 修正瀏覽群組話題清單的快速鍵 (ALT+6)。從聊天清單開啟論壇群組時，現在可以正確偵測到話題清單。
* 新增輸入中提示音效：當對方正在聊天中輸入時，會循環播放音效；對方停止輸入時，音效就會停止。此功能參考自 Unigram JAWS 腳本的相同功能。

### 5.5.0版

* 新增對 NVDA 2026.1 的相容性。

### 5.4.2版

* 新增對 NVDA 2025.3.3 的相容性。

### 5.4.1版

* 新增對 NVDA 2025.1.2 的相容性。
...
* 新增了一個鍵盤快速鍵(預設為 ALT + W)，用來朗讀訊息的傳送時間或接收時間，以及訊息中的反應。當您只想聽取訊息的傳送/接收時間與反應，而不想先重聽完整的訊息內容時，此功能非常有用。連按兩下可切換此資訊的朗讀模式。
* 修正了近期因 Unigram 介面變更而累積的所有問題。
* 將開啟使用者個人資料的鍵盤快速鍵變更為 ALT + Shift + P。
* 將跳至下一個和上一個搜尋結果的鍵盤快速鍵變更為 F3 和 Shift + F3，以符合大多數應用程式的操作習慣。
* 新增了對 NVDA 2024.4 的相容性。

### 5.2.5版

* 已實作附加元件與最新版 Unigram 的相容性。
...
* 新增一個選項用來調整當在空白的訊息編輯區按下方向鍵上時執行的動作。您可以從以下的選項中選擇：編輯上一則傳送的訊息、跳轉到上一則訊息或不值行動做。
* 修正使用鍵盤接聽或拒絕通話。
