...
| **NVDA+Alt+V** | UnigramPlus | Открыть сведения о версиях Unigram и UnigramPlus в окне только для чтения |
...
* Удалён обходной приём для длительности звонка. Unigram 12.10 и новее сами объявляют длительность в сообщении о звонке, поэтому UnigramPlus больше её не добавляет и она не звучит дважды.
* Исправлен сбой запуска дополнения в старых версиях NVDA, в которых нет модуля utils.security.
* Удалены импорты, которые дополнение больше не использует.
* Обновлён вьетнамский перевод.

###Версия 5.8.0

* Периодические фоновые проверки дополнения больше не выполняются в главном цикле NVDA, поэтому навигация по чатам, ввод текста и речь больше не задерживаются, пока Unigram отвечает.
* Все возможности, добавленные после версии 5.4, теперь можно включить или отключить в настройках UnigramPlus; они собраны под одним заголовком: как объявляется кнопка записи голосового сообщения, форматированный текст сообщения по ALT+C, подпись кнопки идентификации в профиле, объявление ответа и редактирования в поле сообщения, отключённое объявление "список" перед сообщениями, текущее состояние микрофона и камеры во время звонка, а также число непрочитанных при переключении папок чатов. Каждая по умолчанию сохраняет текущее поведение, и в каждой подписи указано, какое поведение версии 5.4 она возвращает.
* Исправлено, что пробел не воспроизводил голосовые сообщения, музыку и файлы.
* Исправлено, что Enter не отвечал на сообщение, Backspace не открывал его для редактирования, ALT+C не показывал его текст, а ALT+D не возвращал к нему.
* Исправлено, что ALT+E не закрывал аудиоплеер.
* Исправлено, что Ctrl+ALT+Влево и Ctrl+ALT+Вправо не перематывали воспроизведение голосового сообщения, в том числе при повторном нажатии стрелки с удерживаемыми модификаторами.
* Отмена записи голосового сообщения объявляется примерно через полторы секунды после остановки, а не через пять секунд.
* При выбранном варианте кнопки записи из версии 5.4 фокус остаётся в поле сообщения в начале записи, при этом Tab по-прежнему доходит до кнопки записи.
* Исправлен ряд горячих клавиш, которые молча не срабатывали, когда в дереве Unigram отсутствовал ожидаемый элемент: ALT+3, ALT+4, ALT+5, ALT+6, ALT+Q, ALT+T, ALT+Shift+L, ALT+Shift+P, Shift+Delete, навигация по медиа и управление звонком.
* Преобразование голосового сообщения в текст больше не запускает новый поток каждые полсекунды во время ожидания и прекращает попытки вместо бесконечного ожидания.

###Версия 5.7.3

* NVDA+Alt+V теперь отображает версию UnigramPlus с новой строки.

###Версия 5.7.2

* NVDA+Alt+V теперь открывает сведения о версиях в многострочном окне только для чтения.

###Версия 5.7.1

* Исправлено ошибочное распознавание кнопок реакций в Unigram 12.10.2 как сообщений, из-за которого сочетания для навигации по сообщениям заменяли их штатное действие переключателя.
...
* Исправлена ошибка, которая могла возникнуть при входе NVDA на защищённый рабочий стол, например при появлении запроса UAC, когда было включено фоновое отслеживание сообщений UnigramPlus.
* Исправлено ошибочное распознавание строк тем «Сохранённых сообщений» в Unigram 12.10.1+ как сообщений лишь потому, что их нативные сводки содержат время отправки или получения.
* Удалён устаревший обходной способ именования тем «Сохранённых сообщений»; текущие версии Unigram теперь изначально предоставляют доступные имена для чатов «Сохранённых сообщений».
* Обновлена совместимость с NVDA 2026.2.
* Обновлены польский и бирманский переводы.

### Версия 5.6.8

* Исправлена ошибка Alt+C при установленном WhatsApp Enhancer, вызванная конфликтом имён вспомогательных модулей в appModules.
...
* Добавлено NVDA+Shift+V для объявления установленных версий Unigram и UnigramPlus.
* Удалено временное решение для встроенных кнопок Unigram 12.9 после исправления их меток в Unigram 12.9.1; это предотвращает зависания UIA в списках ботов, при навигации по сообщениям и реакциям, а также в списке чатов.
* Обновлены все переводы и руководства для версии 5.6.6.

### Версия 5.6.5

* Исправлены Enter для ответа на сообщения и Alt+Shift+R для отметки чатов как прочитанных в текущих версиях Unigram.
* Удалены режим веб-представления и его настройка; теперь Alt+C всегда открывает текст сообщения в исходном всплывающем окне wx.
* Обновлены все переводы и руководства для версии 5.6.5.

### Версия 5.6.4

* Исправлена комбинация Shift+Delete: теперь используется штатная команда Delete в Unigram и удаление у обеих сторон подтверждается автоматически.
* Alt+2 теперь сначала нажимает кнопку Unigram «Перейти вниз»; дублирующая комбинация Alt+End удалена.
* Добавлена настройка выбора классического окна wx (по умолчанию) или веб-представления при показе текста сообщения по Alt+C.
* Теперь используется официальный доступный текст форматированных сообщений Unigram; добавлено временное исправление для неподписанных встроенных кнопок в Unigram 12.9.
* Список горячих клавиш обновлён по официальной документации Unigram 12.9.
* Добавлено временное исправление: строки тем «Сохранённых сообщений» теперь объявляют видимое название чата вместо имени типа TDLib.

### Версия 5.6.3

* Исправлена периодически возникавшая проблема, из-за которой NVDA произносил «список» перед сообщением при навигации стрелками вверх и вниз.
* Добавлена настройка для функции Alt+[, которая озвучивает заголовки сообщений после их содержимого; по умолчанию она отключена.
* Добавлено необязательное звуковое уведомление при достижении конца чата.
...
* Исправлено отслеживание прогресса передачи файлов: оно останавливается на 100% и больше не создаёт новый поток при каждом цикле опроса.
* При открытии Unigram фокус теперь автоматически перемещается в список чатов.
* Добавлено Ctrl+Alt+Up/Down для перехода по чатам с непрочитанными упоминаниями.
